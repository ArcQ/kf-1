...
import { zip, expand, filter, map, share, buffer, takeUntil } from 'rxjs/operators';
...
function getBufferedEvent(frames$, event$) {
  const eventPerFrame$ = event$.pipe(buffer(frames$));
...
function createGameLoopState(initialGameState, cancel$) {
  ...
  return {
     ...
     */
    updateGame: updateF => {
      gameState$.next(updateF(gameState$.getValue()));
...
export function createGameLoop(eventSources = [], initialState, cancel$) {
  ...
  const frames$ = of(undefined).pipe(expand(val => calculateStep(val)), filter(frame => frame !== undefined), map(frame => frame.deltaTime), takeUntil(cancel$));
  const bufferedObsList = eventSources.map(obsGetter => getBufferedEvent(frames$, obsGetter));
  return _objectSpread({
    framesAndEvents$: frames$.pipe(zip(...bufferedObsList, (deltaTime, ...args) => ({
      deltaTime,
      inputState: flatten(args)
    })), share())
  }, gameLoopStates);
}
export default {};