...
function combineDicts(requiredDicts) {
  const combinedDict = requiredDicts.reduce((_combinedDict, dictName) => {
    const curDict = Object.keys(dicts[dictName]).map(key => ({
      dictName,
      key,
      assetName: dicts[dictName][key]
    }));
...
export function load({
  ...
  return Observable.create(observer => {
    ...
    combinedDicts.reduce((loader, {
      dictName,
      key,
      assetName
    }) => loadedDicts.indexOf(dictName) === -1 ? loader.add(`${dictName}_${key}`, `${assetUrl}${assetName}`) : loader, PIXI.loader)
    /**
     * Percentage event.
     *
     ...
     */
    .on('progress', loader => observer.next({
      percentage: parseInt(loader.progress, 10)
    })).load(() => {
...
export function getSprite(dictName, key) {
  return new PIXI.Sprite(PIXI.loader.resources[`${dictName}_${key}`].texture);
}
export default {};