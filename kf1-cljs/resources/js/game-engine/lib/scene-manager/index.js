...
import { concat, map, tap, catchError, combineLatest } from 'rxjs/operators';
...
function _createLoadObs(wrappedScene) {
  ...
  const launchLoadingScene$ = tap(null, null, () =>
    ...
    //   }
    // })));
  const setLoadPercentage$ = map(({
    percentage
  }) => {
    engine.ui.dispatch(loadingActions.setLoadPercentage({
      percentage
    }));
    if (wrappedScene.onLoadNext) wrappedScene.onLoadNext();
  });
  const loadAssetPipe$ = loadLoadingAssets$.pipe(launchLoadingScene$, concat(loadSceneAssets$), setLoadPercentage$);
  return forkJoin(loadAssetPipe$, sceneCustomLoad$).pipe(catchError(e => {
...
function _loadScene(wrappedScene) {
  ...
  loadScene$.subscribe(([_, sceneCustomRes]) => {
    //eslint-disable-line
...
function _wrapInSceneHelpers(sceneObj) {
  const wrappedScene = Object.assign({}, sceneObj, {
    ...
    onFinishLoad(sceneCustomRes) {
      const initialState = sceneObj.onFinishLoad ? sceneObj.onFinishLoad(engine.app.stage, sceneCustomRes) : new Map();
      const gameLoopArgs = createGameLoop(sceneObj.eventSources, initialState, _cancelPrevGameLoop$);
      const {
        framesAndEvents$
      } = gameLoopArgs;
      ...
      if (sceneObj.update) {
        framesAndEvents$.pipe(map(combinedRes => sceneObj.update(framesAndEvents$, combinedRes.deltaTime, combinedRes.inputState))).subscribe();
      }
    }
