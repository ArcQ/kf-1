...
components::CMovement systems::MoveHandler::create_movement_component(models::Pt &pos,
  ...
  c_movement.normalized = models::Pt(c_movement.diff.x / abs(c_movement.diff.x),
                                     c_movement.diff.x / abs(c_movement.diff.x));
...
bool systems::MoveHandler::move(double dt,
  ...
  auto move_diff = models::Pt(c_move.multipliers.x * c_move.normalized.x * dist,
